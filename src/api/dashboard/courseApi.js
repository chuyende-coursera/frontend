...
export async function queryUserCourseAllWeb(params) {
  const { filter, sort, range } = params;
  const query = {
    filter: JSON.stringify(filter),
    sort: JSON.stringify(sort || ["id", "DESC"]),
  ...
  return request(`http://localhost:6868/web/userCourse?${stringify(query)}`);
  // return request(`http://localhost:6868/Courses/get/all`);
}
...
export async function addUserCourse(params) {
  return request(`http://localhost:6868/api/userCourse/new`, {
    method: "POST",
    body: {
      ...params,
      method: "post",
