...
class Header extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (Object.keys(nextProps.sessions).length > 0) {
      this.setState({
        authenticated: nextProps.sessions.authenticated,
      });
    }
  }
  ...
  search = async (val) => {
    ...
    setTimeout(
      () =>
        Axios.get(
          `http://localhost:6868/web/courses?filter={"title":"${val}"}`
        ).then((courses) => {
          if (courses.status === 200) {
            this.setState({
              options: courses.data.result.list.map((e) => {
                const url = parseUrI(`/learn/${e.title}`);
                return {
                  label: (
                    <Link
                      to={{
  ...
  render() {
    ...
    const isDashboard = pathname && pathname.split("/").includes("dashboard");
    const menu = (
      <Menu>
        <Menu.Item key="0">
          <Link to="/myCourse">Khóa học của bạn</Link>
        </Menu.Item>
        <Menu.Item key="1">
    ...
    return (
      <Fragment>
        {isDashboard ? null : (
          <nav className="navbar navbar-expand-lg navbar-light bg-white fixMenu">
